...
mus=np.array([0.1,0.3,0.5,0.7,1.0])
for mu in mus:
    cosines=np.load('clv_cosine_mu={}_model_dim={}_num_cl={}.npy'.format(mu,model_dim,num_clv))
    # save these 
    os.chdir(data_path)
    ...
    plt.title('CLV for mu={}'.format(mu))
    plt.yticks(np.arange(0.0,1.1,0.1))
    plt.savefig('clv_box_mu_{}.png'.format(mu))
